 ...
 */
@IncludeJavaScriptLibrary(value = {"${tapestry.scriptaculous}/controls.js", "context:js/slider/slider.js"})
@IncludeStylesheet(value = {"context:js/slider/Slider.css"})
...
/*String jsCommand = "new   Control.Slider('handle_%s','track_%s',{sliderValue:" + getNumberPattern(_value) + ",range:"
+ "$R(" + getNumberPattern(_min) + "," + getNumberPattern(_max) + "),increment:" + getNumberPattern(_inc) + ",onSlide:function(v){ var nv =Math.round(v/this.increment)*this.increment; $('output_%s').innerHTML = nv; $('%s').value=nv; } });";*/
//jsCommand = String.format(Locale.US, jsCommand, getClientId(),getClientId(), _value, _min, _max, _inc, getClientId());
        ...
        String linkUrlStart = getActionLink();
        String linkUrlEnd = "";
        int pos = getUrlSplit( linkUrlStart );
        if ( pos > 0 )
        {
            linkUrlEnd = linkUrlStart.substring( pos );
            linkUrlStart = linkUrlStart.substring( 0, pos ) + "/";
        }
        ...
        //jsCommand = String.format(Locale.US, jsCommand, _ouputId, getActionLink(), "Something went wrong...");
                jsCommand = String.format(Locale.US, jsCommand, _ouputId, linkUrlStart, linkUrlEnd, "Something went wrong...");

        _pageRenderSupport.addScript(jsCommand);
    }

    private int getUrlSplit( String url )
    {
        int p1 = url.indexOf( '?' );
        int p2 = url.indexOf( ';' );
        if ( -1 == p1 ) return p2;
