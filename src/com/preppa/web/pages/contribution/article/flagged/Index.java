package com.preppa.web.pages.contribution.article.flagged;
...
public class Index {
   ...
   @Validate("required")
    ...
    void onActivate() {
        flags = flagDAO.FindAllByContentType(ContentType.Article);
    ...
    Block onChangeFromFlagSelect(String selected) {
       ...
       if(selected != null && !selected.equals(""))
       {
          ...
          // Active, Incorrect, Spam, Inappropriate, Copyright, Attention
           ...
           {
               flags = flagDAO.FindAllByFlagType(ContentType.Article, ContentFlag.Incorrect);
           }
           else  if(selected.equals("Spam"))
           {
               flags = flagDAO.FindAllByFlagType(ContentType.Article, ContentFlag.Spam);
           }
           else if(selected.equals("Inappropriate"))
           {
               flags = flagDAO.FindAllByFlagType(ContentType.Article, ContentFlag.Inappropriate);
           }
           else if(selected.equals("Copyright"))
           {
               flags = flagDAO.FindAllByFlagType(ContentType.Article, ContentFlag.Copyright);
           }
           else  if(selected.equals("Attention"))
           {
               flags = flagDAO.FindAllByFlagType(ContentType.Article, ContentFlag.Attention);
           }
